...
tape( 'attached to the main export is a read-only `of` method', function test( t ) {
	t.ok( hasOwnProp( Int64, 'of' ), 'has property' );
	t.ok( isFunction( Int64.of ), 'has method' );
...
tape( 'the method throws an error if not provided two unsigned 32-bit integers', function test( t ) {
	...
	function badValue( value1, value2 ) {
		return function badValue() {
			var x = Int64.of( value1, value2 ); // eslint-disable-line no-unused-vars
		};
	}
});
...
tape( 'the method returns a 64-bit signed integer', function test( t ) {
	...
	x = Int64.of( UINT32_MAX, UINT32_MAX );
	t.ok( x instanceof Int64, 'returns expected value' );
