...
tape( 'the constructor throws an error if provided an invalid/unsupported BigInt', opts, function test( t ) {
	var values;
	var i;
...
tape( 'the constructor returns a 64-bit signed integer', function test( t ) {
	...
	x = new Int64( MAX_SAFE_INTEGER + 1 );
	t.ok( x instanceof Int64, 'returns expected value' );
	t.strictEqual( x.hi, 2097152, 'returns expected value' );
	t.strictEqual( x.lo, 0, 'returns expected value' );

	x = new Int64( MIN_SAFE_INTEGER - 1 );
	t.ok( x instanceof Int64, 'returns expected value' );
	t.strictEqual( x.hi, 4292870144, 'returns expected value' );
	t.strictEqual( x.lo, 0, 'returns expected value' );

	x = new Int64( TWO_63 - 1024 ); // Largest integer value less than 2^63 exactly representable as a double
	t.ok( x instanceof Int64, 'returns expected value' );
	t.strictEqual( x.hi, 2147483647, 'returns expected value' );
	t.strictEqual( x.lo, 4294966272, 'returns expected value' );

	x = new Int64( -TWO_63 ); // Smallest integer value on the interval [-(2^63), 2^63-1]
	t.ok( x instanceof Int64, 'returns expected value' );
	t.strictEqual( x.hi, 2147483648, 'returns expected value' );
	t.strictEqual( x.lo, 0, 'returns expected value' );

	x = new Int64( -TWO_63 + 1024 );
	t.ok( x instanceof Int64, 'returns expected value' );
	t.strictEqual( x.hi, 2147483648, 'returns expected value' );
	t.strictEqual( x.lo, 1024, 'returns expected value' );
